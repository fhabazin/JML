...
public class NeuralNetwork {
	...
	public NeuralNetwork(double[][] trainingSet, double[][] testSet,
		...
		for (int i = 0; i < trainingSet.length; i++) {
			...
			tempKnown[i] = trainingSet[i][trainingSet[0].length];
		}
		...
		temp = new double[testSet.length + 1][trainingSet[0].length];
		tempKnown = new double[testSet.length + 1];
		identity = getIdentityMatrix(this.trainingSet, trainingSetKnown);
		...
		for (int i = 0; i < testSet.length; i++) {
			...
			tempKnown[i] = testSet[i][trainingSet[0].length];
		}
	...
	private CostFunction getCostFunction() {
		CostFunction costFunction = new CostFunction() {
			...
			public CostGradientTuple evaluateCost(DoubleVector input) {
				...
				for (int i = 1; i < unroledTheta.length; i++) {
					if (i == 0)
